...
For typography outside `Text`, use the documented `type-nesra-*` role classes and `text-nesra-*` for semantic text color. A text color does not set a type role. When passing custom classes into a Nesra component or through its exported `cn` merger, inspect the resulting class string if a style disappears. Similar prefixes can make distinct design-system utilities conflict. For a visual regression, also check that Tailwind emitted the expected CSS.
